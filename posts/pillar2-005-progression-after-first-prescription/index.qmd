...
This piece assumes you already have a running prescription (intensity, weekly volume, school/sport steps). The [series topic table in installment 007](../pillar2-007-prescription-path-topology/index.html) lists where each of those was covered, if you need a map. **What none of those pieces answered:** once a prescription is running, what changes it—and does that rule still hold when school and sport are both active?

No trial defines same-day school-plus-aerobic budgeting, so the [SOP checklist](#sop-dual-track) below relies on clinical judgment rather than a published rule. No adolescent PPCS trial has randomized progression-rule type either. This piece maps three published ways to advance a prescription, then reads the Berlin and Amsterdam consensus PDFs to show that school and sport steps do not share the same timing rules ([consensus table audit](#consensus-table-audit); Figure 2).
...
Putukian’s pooled RTL mean (**8.3 d**) sits above the CISG thought-experiment minimum (**4 d**)—a coarse reminder that real-world “days until RTL” in ~278 pooled studies is not the same construct as Amsterdam step labels. **Do not treat Putukian’s RTP:RTL ratio (2.39) as a harmonized dual-track statistic:** *I²* 99.3% means numerator and denominator came from study-specific endpoints never step-mapped to CISG tables. For whether RTL documentation **lags** RTP in practice, Carson’s dual-track chart review ([004](../pillar2-004-school-vs-sport-staging/index.html)—RTL premature return unchanged while RTP improved) and DeMatteo’s youth medians (**35 vs 38 d** on full staged-protocol completion) are the cleaner read.
...
if (requireNamespace("patchwork", quietly = TRUE)) {
 ...
 "RTS:RTL ratios listed for orientation only — not merged evidence (Putukian I² 99.3%)."
 ),
 theme = theme(
...
The next installment turns to the monitoring channel itself: whether a wrist optical heart-rate reading can substitute for the chest strap these prescriptions were built on. The [RSS feed](../../index.xml) on the hub will notify readers when it publishes.
